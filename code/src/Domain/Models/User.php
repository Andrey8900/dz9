...
class User {
    ...
    public function updateUser(array $data): void {
        ...
        if (isset($data['user_name'])) {
            ...
            $params['user_name'] = htmlspecialchars($data['user_name']);
        }
    
        if (isset($data['user_lastname'])) {
            $fields[] = 'user_lastname = :user_lastname';
            $params['user_lastname'] = htmlspecialchars($data['user_lastname']);
        }
    
        if (!empty($fields)) {
            $sql = "UPDATE users SET " . implode(', ', $fields) . " WHERE id_user = :id_user";
            $handler = Application::$storage->get()->prepare($sql);
